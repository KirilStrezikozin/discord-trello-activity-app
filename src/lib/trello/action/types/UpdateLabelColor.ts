...
export default class ActionUpdateLabelColor extends LabelActionBase {
  ...
  protected override data?: z.infer<typeof ActionUpdateLabelColor.schema>;

  protected override buildMessageInner(
    embed: EmbedBuilder, opts: MessageOptions
  ): EmbedBuilder {
    const name = opts.member
      ? `${opts.member?.username} has changed a label color`
      : "A label color has been changed";

    const oldColor = this.data!.data.old.color;
    ...
    embed
      ...
      .addFields({
        ...
        value: oldColor ? oldColor : "None",
        inline: true
      })
      ;

    this.buildLabelColorField(embed, "New Label Color", true);
