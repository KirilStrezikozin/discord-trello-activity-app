...
export default class ActionMovedCheckItemLower extends CheckListActionBase {
  public static override readonly schema = z.object({
    id: z.string().min(1),
    type: z.literal("updateCheckItem"),
    translationKey: z.literal("action_moved_checkitem_lower"),
  ...
  protected override data?: z.infer<typeof ActionMovedCheckItemLower.schema>;

  protected override buildMessageInner(
    embed: EmbedBuilder, opts: MessageOptions
  ): EmbedBuilder {
    const name = opts.member
      ? `${opts.member?.username} has moved an item lower in a checklist in a card`
      : "An item has been moved lower in a checklist in a card";
