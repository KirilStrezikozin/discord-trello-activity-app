...
export default class ActionMakeAdminOfBoard extends Action {
  public static override readonly schema = z.object({
    id: z.string().min(1),
    type: z.literal("makeAdminOfBoard"),
  ...
  protected override data?: z.infer<typeof ActionMakeAdminOfBoard.schema>;

  protected override buildMessageInner(
    embed: EmbedBuilder, opts: MessageOptions
  ): EmbedBuilder {
    const name = opts.member
      ? `${opts.member?.username} has changed a member role to admin`
      : "A member role has been changed to admin";
