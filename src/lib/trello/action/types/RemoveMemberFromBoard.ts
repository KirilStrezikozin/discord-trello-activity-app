...
export default class ActionRemoveMemberFromBoard extends Action {
  public static override readonly schema = z.object({
    id: z.string().min(1),
    type: z.literal("removeMemberFromBoard"),
  ...
  protected override data?: z.infer<typeof ActionRemoveMemberFromBoard.schema>;

  protected override buildMessageInner(
    embed: EmbedBuilder, opts: MessageOptions
  ): EmbedBuilder {
    const name = opts.member
      ? `${opts.member?.username} has removed a member from a board`
      : "A member has been removed from a board";
