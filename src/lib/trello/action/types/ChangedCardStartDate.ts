...
export default class ActionChangedCardStartDate extends Action {
  static schema = z.object({
    ...
    translationKey: z.literal("action_changed_a_start_date"),
  ...
  private data?: z.infer<typeof ActionChangedCardStartDate.schema>;

  static override from(data: unknown): ActionBuildResult {
    const res = ActionChangedCardStartDate.schema.safeParse(data);
    if (!res.success) {
      return {
        success: false,
        action: null,
    ...
    const action = new ActionChangedCardStartDate();
  ...
  buildMessage(opts: MessageOptions): (string | MessagePayload | WebhookMessageCreateOptions) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFields(
        { name: "Previous Start Date", value: this.data!.data.old.start, inline: true },
