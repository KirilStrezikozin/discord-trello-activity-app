...
export default class ActionMakeNormalMemberOfBoard extends Action {
  public static override readonly schema = z.object({
    id: z.string().min(1),
    type: z.literal("makeNormalMemberOfBoard"),
  ...
  protected override data?: z.infer<typeof ActionMakeNormalMemberOfBoard.schema>;

  protected override buildMessageInner(
    embed: EmbedBuilder, opts: MessageOptions
  ): EmbedBuilder {
    const name = opts.member
      ? `${opts.member?.username} has changed a member role to normal`
      : "A member role has been changed to normal";
